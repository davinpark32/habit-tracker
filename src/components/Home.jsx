...
export default function Home({ onComplete, onAdd, onOpenDate }) {
  const { today, goalsOn, streak, growthStage, undoGoal, removeGoal, willCompleteDay, dayClearSeen } = useStore()
  ...
  return (
    <section className={stickyHome ? 'screen sticky-home' : 'screen'}>
      ...
      <div className="home-scroll">
        ...
        <div className="cards home-cards">{goals.map((goal) => <GoalCard key={goal.id} goal={goal} onComplete={() => complete(goal)} onUndo={() => undoGoal(goal.id, today)} onTap={onOpenDate} onDelete={(scope) => removeGoal(goal.id, today, scope)} />)}</div>
