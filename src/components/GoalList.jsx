...
export default function GoalList({ date, onBack, onComplete, onEdit, onAdd }) {
  const { goalsOn, isPast, today, undoGoal, removeGoal, willCompleteDay, dayClearSeen } = useStore()
  ...
  return (
    <section className="screen">
      ...
      <div className="cards">{goals.map((goal) => <GoalCard key={goal.id} goal={goal} locked={past} onComplete={() => complete(goal)} onUndo={() => undoGoal(goal.id, date)} onEdit={() => onEdit(goal)} onDelete={(scope) => removeGoal(goal.id, date, scope)} />)}</div>
